...
To compile the opt-in v2 file-level DFU candidate, extract the pinned source and use a separate output directory. The script checks that the file RPC object was compiled and writes a padded `experimental-file-rpc-loader.bin` plus SHA-256 `manifest.json` beside the output directory. It refuses to overwrite either artifact. The resulting image is separate from `assets/loader.bin` and is not included in the normal toolkit package. Replace the host path below with your own matching Buildroot `output/host` directory.

From the toolkit repository root:

```sh
mkdir -p /tmp/jibo-loader-source
...
python3 scripts/build_loader.py \
  ...
  --out .build/file-level-v2/source \
...
python3 scripts/build_file_level_entry.py \
  ...
  --out .build/file-level-v2/shofel-entry
```
