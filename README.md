...
After DFU appears, **Check partition layout (read-only)** in the menu or `sudo python3 jibo_dfu.py probe-dfu-gpt --port 1-1` is intended to read and validate only the first 32 KiB of eMMC. This action is temporarily disabled: `dfu-util -Z` sets the progress scale but does not limit the upload, so the toolkit refuses to start the read until a bounded DFU reader is installed.
